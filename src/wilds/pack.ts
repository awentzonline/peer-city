...
const ON_BACK = new THREE.Vector3(0, -0.28, 0.24);
...
export class VrPack {
  ...
  private readHand(hand: XRHand): void {
    ...
    if (grip.distanceTo(this.bag.getWorldPosition(this.local)) < REACH) {
      this.open = !this.open;
    ...
    for (const slot of this.slots) {
      if (grip.distanceTo(slot.group.getWorldPosition(this.local)) > SLOT_REACH) continue;
