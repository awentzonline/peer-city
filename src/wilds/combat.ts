...
export function registerCombat(ctx: WildsContext, survivor: SurvivorRole): void {
  ...
  world.onCommand(Fuel, Campfire, (fire, p) => applyFuel(ctx, fire, p.seconds));

  world.onCommand(Hunted, Survivor, (hunter, p) => {
    if (hunter === ctx.me) ctx.hud.message(`You brought down a ${animalSpec(p.kind).name}. Butcher it with the axe.`);
  });

  world.onCommand(Butcher, Animal, (a) => {
    if (a.state.mode !== AnimalMode.Dead || a.state.meat === 0) return;
    a.state.meat--;
    const angle = Math.random() * Math.PI * 2;
    world.spawn(Item, { x: a.state.x + Math.cos(angle) * 0.8, y: a.state.y + Math.sin(angle) * 0.8, tool: RAW_MEAT.id, amount: 1 });
  });

  world.onCommand(Damage, Animal, (target, p) => {
    ...
    if (s.hp === 0) {
      ...
      s.meat = spec.meat;
      s.target = 0;
      if (attacker) world.command(Hunted, { attacker: p.attacker, kind: s.kind });
  ...
  world.onCommand(Damage, Survivor, (target, p) => {
    const s = target.state;
