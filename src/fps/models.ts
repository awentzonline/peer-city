 ...
 * few meshes, so every human or car costs a handful of draw calls and shares
 * one material. Models face +X (a heading of 0) with +Z on their right.
 */
...
export function buildHuman(look: HumanLook): HumanRig {
  const key = `${look.shirt}:${look.pants}:${look.skin}:${look.hair}:${look.cop}`;
  const torso = cached(`torso:${key}`, () =>
    merge([
      box(0.24, 0.22, 0.42, 0, 0.99, 0, look.pants),
      box(0.26, 0.56, 0.48, 0, 1.3, 0, look.shirt),
      ...(look.cop ? [box(0.02, 0.07, 0.08, 0.135, 1.43, -0.13, 0xffd54a)] : []),
    ]),
  );
  const head = cached(`head:${key}`, () =>
    merge([
      ...
      ...(look.cop
        ? [box(0.28, 0.07, 0.28, 0, 0.28, 0, 0x1f2a52), box(0.1, 0.02, 0.26, 0.16, 0.25, 0, 0x1f2a52)]
  ...
  root.add(body);
  body.add(new THREE.Mesh(torso, MAT.solid));
  const pivot = (geo: THREE.BufferGeometry, x: number, y: number, z: number) => {
    const p = new THREE.Group();
    p.position.set(x, y, z);
    p.add(new THREE.Mesh(geo, MAT.solid));
    body.add(p);
    return p;
  };
