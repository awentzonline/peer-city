...
export function updateOwnedPeds(ctx: GameContext, dt: number): void {
  ...
  for (const p of world.all(Ped)) {
    ...
    if (s.mode === PedMode.Flee) {
      ...
      if (moveCircle(city, s, Math.cos(a) * RUN_SPEED * dt, Math.sin(a) * RUN_SPEED * dt, PED_RADIUS)) {
        // cornered: veer along the wall
        l.fx = s.x - Math.cos(a + 1.2) * 4.5;
        l.fy = s.y - Math.sin(a + 1.2) * 4.5;
      }
      s.angle = a;
      continue;
    }
    ...
    if (d < 0.25 || (s.tx === 0 && s.ty === 0)) {
      chooseNextTile(city, p);
      continue;
    }
    const step = Math.min(d, WALK_SPEED * dt);
    moveCircle(city, s, (dx / d) * step, (dy / d) * step, PED_RADIUS);
