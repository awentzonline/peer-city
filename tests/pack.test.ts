...
/** The pack on the back, and where its contents float in front of the chest when it's open (pack.ts). */
const ON_BACK = handFor(0, -0.28, 0.24);
...
function setup() {
  ...
  const inv = new Inventory<WildTool>(new Toolbox([AXE, LOGS]) as Toolbox<WildTool>);
